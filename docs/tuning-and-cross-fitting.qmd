...
The `Pipeline` + `GridSearchCV` pattern is Python-only. From R, call into Python via `reticulate` using the same code: pass an R data frame with `reticulate::r_to_py(df)` and read the result back with `as.numeric(reticulate::py_to_r(...))`.
:::
