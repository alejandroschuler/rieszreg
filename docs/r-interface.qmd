...
R6's `$method()` syntax maps to Python's `.method()`. Integer-typed kwargs (`n_estimators`, `max_depth`, `random_state`, `early_stopping_rounds`, `n_landmarks`, `n_features`, `cg_max_iter`) take an `L` suffix in R: `n_estimators = 200L`. Other kwargs are auto-converted by reticulate.
