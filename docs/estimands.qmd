...
The two pieces a `FiniteEvalEstimand` carries:

```python
@dataclass
class FiniteEvalEstimand(Estimand):
    feature_keys: tuple[str, ...]   # column names α is indexed by
    m: Callable[..., Any]           # the operator m(alpha)(z, y)
    name: str = "custom"
    factory_spec: dict | None = None
```

`feature_keys` declares the input schema of $\alpha$ — the columns the underlying learner consumes when scoring a row. The orchestrator slices these columns out of `X` per row to build the feature matrix the learner sees.

The outcome `Y` is **not** part of `feature_keys`. It flows in the sklearn-style way: a separate vector passed as the second argument to `.fit(X, y)`. The orchestrator threads it through to `m(alpha)(z, y)`. Built-in estimands ignore `y`; pass it anyway when following sklearn convention.
...
| Outcome $Y$ | `re78` | `.fit(X, y=df["re78"])`. |

The `X` dataframe must contain every name in `feature_keys`; extra columns are ignored. With an `ndarray`, columns are interpreted in `feature_keys` order.
...
Some estimands need the per-row outcome $y$ to evaluate $m(\mu)(z, y)$. The most common pattern is a contrast multiplied by an indicator on $Y$ — for example, the average treatment effect *on rows with $Y$ above a fixed threshold $\tau$*:
$$
\psi = \mathbb{E}\!\left[\,\mathbf{1}\{Y > \tau\}\,\big(\mu(1, X) - \mu(0, X)\big)\,\right].
$$
This is a partial-estimand surface (same caveat as `ATT`). The corresponding operator reads `y`:
...
est = FiniteEvalEstimand(
    feature_keys=("a", "x"),
    m=m_upper_half_ate,
    name="UpperHalfATE_partial",
)
```

Trace it on a row with `y > tau` (active) and on a row with `y <= tau` (inactive):

```{python}
trace(est, {"a": 0.0, "x": 0.5}, y=0.9)
```

```{python}
trace(est, {"a": 0.0, "x": 0.5}, y=-0.5)
```

The active row generates an ATE-style finite linear combination; the inactive row collapses to nothing because the indicator is zero. `build_augmented` and `RieszBooster.fit(X, y)` plumb `y` through automatically:
...
booster = RieszBooster(
    estimand=est, n_estimators=100, max_depth=3, learning_rate=0.05,
).fit(df, y)
booster.predict(df)[:5]
```

::: {.callout-note title="Canonical literature examples"}
Several papers (Hines & Miles, Chernozhukov, Kato) catalog functionals where $m$ depends on $Y$ directly. We will add a vetted reference example here once a paper PDF is in `reference/`.
:::
