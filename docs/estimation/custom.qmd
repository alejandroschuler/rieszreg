...
Cross-fit $\hat\alpha$ from `rieszreg` plus a cross-fit outcome regression $\hat\mu$ are the two ingredients of a one-step / DML / TMLE estimator for any linear-functional target. This page builds both estimators on the Lalonde NSW dataset in ~50 lines.
...
X_full = df[["a"] + COVARIATES].to_numpy(dtype=float)
...
mu_oof = cross_val_predict(mu_hat, X_full, y, cv=cv)
...
mu_full = mu_hat.fit(X_full, y)
X1 = np.column_stack([np.ones(len(df)),  df[COVARIATES].to_numpy(dtype=float)])
X0 = np.column_stack([np.zeros(len(df)), df[COVARIATES].to_numpy(dtype=float)])
mu1 = mu_full.predict(X1)
mu0 = mu_full.predict(X0)
```
...
```{r}
library(xgboost)
sk_ms <- import("sklearn.model_selection", convert = FALSE)
cv    <- sk_ms$KFold(n_splits = 5L, shuffle = TRUE, random_state = 0L)

# Outcome regression μ̂(A, X) via xgboost (R) with manual 5-fold cross-fit.
X_full <- as.matrix(df[, c("a", COVARIATES)])
y      <- df$y
folds  <- split(seq_len(nrow(df)),
                sample(rep_len(1:5, nrow(df))))
mu_oof <- numeric(nrow(df))
for (te in folds) {
  bst <- xgb.train(
    params = list(objective = "reg:squarederror", eta = 0.05,
                  max_depth = 4, lambda = 1.0, verbosity = 0),
    data = xgb.DMatrix(X_full[-te, , drop = FALSE], label = y[-te]),
    nrounds = 300
  )
  mu_oof[te] <- predict(bst, X_full[te, , drop = FALSE])
}

# Riesz representer α̂ via cross_val_predict on the rieszboost R6 wrapper.
...
alpha_hat <- as.numeric(py_to_r(
  sk_ms$cross_val_predict(booster$py,
                          r_to_py(df[, c("a", COVARIATES)]),
                          cv = 5L)
))
...
$\mathrm{IF}(O) = m(Z, \hat g) + \hat\alpha(Z)\,(Y - \hat\mu(Z))$,
with $m(Z, \hat g) = \hat\mu(1, X) - \hat\mu(0, X)$. Average it for $\hat\theta$; its sample variance gives the SE.
...
The pattern generalizes verbatim. Swap the estimand factory and the $m(Z, \hat g)$ formula:

| Estimand | $m(Z, \hat g)$ | Riesz factory |
|---|---|---|
| ATT | $A(\hat g(1, X) - \hat g(0, X))$, then divide by $\hat P(A=1)$ | `ATT(...)` |
| TSM at $a^\star$ | $\hat g(a^\star, X)$ | `TSM(level=a_star)` |
| Additive shift $\delta$ | $\hat g(A + \delta, X) - \hat g(A, X)$ | `AdditiveShift(delta)` |
| Stochastic intervention | $\int \hat g(a', X)\,h(a' \mid X)\,da'$ | `StochasticIntervention(...)` |

Worked examples for each: [`rieszboost/examples/`](https://github.com/rieszreg/rieszboost/tree/main/examples) (`tsm.py`, `stochastic_intervention.py`, `lee_schuler/binary_dgp.py`).
