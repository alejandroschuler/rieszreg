...
KLLoss is the right choice for density-ratio targets like TSM. The exp link keeps predictions positive without any post-hoc clipping.
...
`RieszNet` is a `BaseEstimator`, so it composes with `clone`, `GridSearchCV`, `cross_val_predict`, `Pipeline`. See [sklearn integration](../sklearn-integration.qmd).
