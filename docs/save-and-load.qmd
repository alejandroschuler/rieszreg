...
- **`estimator.save(path)` / `Cls.load(path)`** — a directory with the backend's binary payload (xgboost UBJSON, joblib pickle, or kernel coefficients) plus a `metadata.json` sidecar. Cross-language: R uses the same format.
...
Custom (non-built-in) `Estimand`s can't be reconstructed from JSON — the user's `m()` callable doesn't survive a save/load cycle. The estimator file itself is fine; you just have to re-pass the estimand on load:
...
- `predictor_kind`: `"xgboost"` (rieszboost), `"sklearn"` (rieszboost), `"krrr"` (krrr), `"forestriesz"` (forestriesz moment-style), or `"aug-forestriesz"` (forestriesz augmentation-style).
