...
c-block (a=0 counterfactuals) is added analytically at prediction time
(γ_c = − b_c / (2 n λ)). Note that the o-block rhs depends on K_oc b_c,
...
def solve_falkon(
    ...
    *,
    ...
    if not np.allclose(a_o, 1.0):
        raise NotImplementedError(
            "solver='falkon' currently assumes a_o ≡ 1 (the standard "
            "augmentation case). For non-uniform a_o, use solver='nystrom_cg'."
        )
    ...
    for lam in lambdas:
        ...
        # y = -0.5 b_o (independent of λ, ignoring K_oc), then add the c-block
        # contribution analytically at prediction time.
        # NOTE: this drops the K_oc b_c / (2 n λ) coupling on the o-system.
